...
extends Module {
    ...
    Value<Boolean> swordGapWeakness = new ValueBuilder().withDescriptor("Sword Gap Weakness").withValue(false).register(this);
    ...
    public Item getItem() {
        ...
        if (Offhand.mc.player.getHealth() + Offhand.mc.player.getAbsorptionAmount() > (float)this.totemHealth.getValue().intValue()) {
            i = this.swordGap.getValue().booleanValue() && Offhand.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem().equals(Items.DIAMOND_SWORD) ? (!this.swordGapWeakness.getValue().booleanValue() && !Mouse.isButtonDown((int)1) ? this.getSelectedItem() : Items.GOLDEN_APPLE) : this.getSelectedItem();
        }
        return i;
    }
