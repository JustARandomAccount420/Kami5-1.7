...
public class Composer {
    ...
    public Node getNode() {
        ...
        this.parser.getEvent();
        Node node = this.composeNode(null, this.blockCommentsCollector.collectEvents().consume());
        this.blockCommentsCollector.collectEvents();
    ...
    private Node composeNode(Node parent, List<CommentLine> blockComments) {
        ...
        if (this.parser.checkEvent(Event.ID.Alias)) {
            ...
            String anchor = event.getAnchor();
            node = this.parser.checkEvent(Event.ID.Scalar) ? this.composeScalarNode(anchor, blockComments) : (this.parser.checkEvent(Event.ID.SequenceStart) ? this.composeSequenceNode(anchor, blockComments) : this.composeMappingNode(anchor, blockComments));
        }
